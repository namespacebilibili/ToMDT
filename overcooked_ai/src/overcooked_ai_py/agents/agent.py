...
class HumanAgent(Agent):
    ...
    def update_pygame(self, surface):
    ...
    def action(self, state):
        surface = self.visualizer.display_rendered_state(state, grid=self.mdp.terrain_mtx, dynamic_display=True)
        if self.not_init:
            self.pygame_init(surface)
            self.not_init = False
        self.update_pygame(surface)
        # check for keypresses, if press K_UP, K_DOWN, K_LEFT, K_RIGHT, K_RETURN, K_SPACE, return corresponding action
        while True:
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_UP:
                        return Action.ALL_ACTIONS[0], {}
                    elif event.key == pygame.K_DOWN:
                        return Action.ALL_ACTIONS[1], {}
                    elif event.key == pygame.K_LEFT:
                        return Action.ALL_ACTIONS[3], {}
                    elif event.key == pygame.K_RIGHT:
                        return Action.ALL_ACTIONS[2], {}
                    elif event.key == pygame.K_RETURN:
                        return Action.ALL_ACTIONS[4], {}
                    elif event.key == pygame.K_SPACE:
                        return Action.ALL_ACTIONS[5], {}
...
class AgentPair(AgentGroup):
    ...
    def __init__(self, *agents, allow_duplicate_agents=False, dt=0):
        super().__init__(
    ...
    def update(self, joint_action_and_infos, r_t):
        if self.dt == 1:
            self.a0.update(joint_action_and_infos[0][0], joint_action_and_infos[1][0], r_t)
        elif self.dt == 2:
            self.a1.update(joint_action_and_infos[1][0], joint_action_and_infos[0][0], r_t)
        elif self.dt == 3:
            self.a0.update(joint_action_and_infos[0][0], joint_action_and_infos[1][0], r_t)
            self.a1.update(joint_action_and_infos[1][0], joint_action_and_infos[0][0], r_t)

class DTAgentPair(AgentGroup):
    """
    AgentPair is the N=2 case of AgentGroup. Unlike AgentGroup,
    it supports having both agents being the same instance of Agent.
    ...
    def joint_action(self, state):
        ...
        self.a1.set_agent_index(1)
        action_and_infos_1 = self.a1.action(state)
        self.a0.update(action_and_infos_0[0], action_and_infos_1[0])
        self.a1.update(action_and_infos_1[0], action_and_infos_0[0])
        joint_action_and_infos = (action_and_infos_0, action_and_infos_1)
        return joint_action_and_infos
        # else:
...
class ToMAugmentedAgent(Agent):
    """
    Only support PPOAgent (RLlibAgent)
    """
    def __init__(self, model, agent, agent_index, featurize_fn, max_len=10, beta = 2, target_return=100):
        self.agent = agent
        self.agent_index = agent_index
    ...
    def action_probabilities(self, state):
        ...
        original_action_probs, value = self.agent.action_probabilities(state, PPO=True)
        state = self.featurize_fn(state)[self.agent_index]
        self.history["ep_states"].append(state)
        self.history["ep_rtgs"][-1] = max(self.history["ep_rtgs"][-1], value[0])
        self.history["ep_timesteps"].append(self.time)
        self.time += 1
        history = deepcopy(self.history)
        history["ep_action0s"].append(6)
        history["ep_action1s"].append(6)

        _, action1_preds, _ = self.model.get_action(
            history["ep_states"],
            history["ep_action0s"],
            history["ep_action1s"],
            history["ep_rtgs"],
            history["ep_timesteps"]
        )
        action1_probs = torch.softmax(action1_preds, dim=0).cpu().detach().numpy()
        action1 = np.random.choice(
            np.array(Action.ALL_ACTIONS, dtype=object), p=action1_probs
        )
        history["ep_action1s"][-1] = Action.ACTION_TO_INDEX[action1]
        history["ep_action0s"].append(6)
        history["ep_action1s"].append(6)
        history["ep_timesteps"].append(self.time)
        history["ep_rtgs"].append(0)
        state_pad = np.zeros_like(state)
        history["ep_states"].append(state_pad)

        tom_prob = np.zeros(len(Action.ALL_ACTIONS))
        for i in range(len(Action.ALL_ACTIONS)):
            history["ep_action0s"][-2] = i
            _, _, rtg_preds = self.model.get_action(
                history["ep_states"],
                history["ep_action0s"],
                history["ep_action1s"],
                history["ep_rtgs"],
            ...
            tom_prob[i] = rtg_preds.item()
        gamma = 0.9
        tom_prob = self.history["ep_rtgs"][-1] - (1 - gamma) * tom_prob
        tom_prob = tom_prob - np.max(tom_prob)
        tom_prob = softmax(tom_prob, beta=self.beta)
        tom_prob = tom_prob / (np.sum(tom_prob))
        # print(tom_prob)

        action_prob = original_action_probs[0] * tom_prob
        action_prob = action_prob / (np.sum(action_prob))
        # print(f"Original: {original_action_probs[0]} v.s. Tom: {tom_prob} v.s modified: {action_prob}")
    
        # action = np.random.choice(
        #     np.array(Action.ALL_ACTIONS, dtype=object), p=action_prob
        # )
        # agent_action_info = {"action_probs": action_prob}
        return action_prob
    ...
    def update(self, self_action, teammate_action, reward):
        self.history["ep_action0s"].append(Action.ACTION_TO_INDEX[self_action])
        self.history["ep_action1s"].append(Action.ACTION_TO_INDEX[teammate_action])
