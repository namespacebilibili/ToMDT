...
class RlLibAgent(Agent):
    ...
    def action_probabilities(self, state, PPO=False):
        """
        Arguments:
        ...
        output = self.policy.compute_actions(
            np.array([my_obs]), self.rnn_state
        )
        logits = output[2]["action_dist_inputs"]
        value = output[2]["vf_preds"]
...
def load_trainer(save_path, true_num_workers=False):
    ...
    if os.path.exists(os.path.join(save_path, "checkpoint_000500")):
        trainer.restore(os.path.join(save_path, "checkpoint_000500"))
    else:
        trainer.restore(os.path.join(save_path, "checkpoint_000450"))
    return trainer
