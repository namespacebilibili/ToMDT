...
class RllibPPOModel(TFModelV2):
    ...
    def forward(self, input_dict, state=None, seq_lens=None, return_value=False):
        if return_value:
            model_out, self._value_out = self.base_model.predict(input_dict["obs"])
            return model_out, self.value_function()
