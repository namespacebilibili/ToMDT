...
if __name__ == "__main__":
    ...
    for layout in [
        ...
        # this is where
        proxy_dir = os.path.join(current_file_dir, "proxy", "train", layout)
        ...
        train_bc_model(proxy_dir, bc_params, True)

    current_file_dir = os.path.dirname(os.path.abspath(__file__))
    # this is where
    bc_dir = os.path.join(current_file_dir, "bc_runs", "train", "all")
    params_to_override = {
        "layouts": ["cramped_room", "asymmetric_advantages", "coordination_ring", "random0", "random3"],
        "layout_name": "all",
        "data_path": CLEAN_2019_HUMAN_DATA_TRAIN,
        "epochs": 100,
        "old_dynamics": True,
